...
function createBond(start, end) {
  ...
  const material = new THREE.MeshPhysicalMaterial({ color: 0xffcbe5, clearcoat: 0.1 });
...
function onMouseMove(event) {
  ...
  if (intersects.length > 0) {
    ...
    tooltip.innerHTML = `${data.name}<br>Atomic Number: ${data.atomicNumber}<br>Atomic Weight: ${data.atomicWeight}`;
...
let mouseX = 0;
let mouseY = 0;
document.addEventListener('mousedown', e => { if (e.button === 0) mouseDown = true; mouseX = e.clientX; mouseY = e.clientY; });
document.addEventListener('mouseup', e => { if (e.button === 0) mouseDown = false; });
document.addEventListener('contextmenu', e => e.preventDefault());
document.addEventListener('mousemove', e => {
  const deltaX = e.clientX - mouseX;
  const deltaY = e.clientY - mouseY;
  if (mouseDown) { molecule.rotation.y += deltaX * 0.01; molecule.rotation.x += deltaY * 0.01; }
  mouseX = e.clientX; mouseY = e.clientY;
});
document.addEventListener('dblclick', () => { molecule.rotation.set(0, 0, 0); camera.position.set(0, 0, 8); });
document.addEventListener('wheel', e => { camera.position.z += e.deltaY * 0.01; camera.position.z = Math.max(4, Math.min(camera.position.z, 20)); });
...
let lastTouchX = 0;
let lastTouchY = 0;
document.addEventListener('touchstart', e => { if (e.touches.length === 1) { touchDown = true; lastTouchX = e.touches[0].clientX; lastTouchY = e.touches[0].clientY; } });
...
document.addEventListener('touchend', () => { lastPinchDistance = null; });
